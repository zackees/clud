...
def _is_interrupt_exit_code(returncode: int) -> bool:
    """Check if return code indicates keyboard interrupt.

    Checks for common interrupt exit codes across platforms:
    - 130: Unix/Linux Ctrl-C exit code (SIGINT)
    - 255: Generic interrupt code (some shells)
    - 3221225786: Windows STATUS_CONTROL_C_EXIT (0xC000013A)
    - -2, 4294967294: Alternative SIGINT representations

    Returns:
        True if returncode indicates keyboard interrupt
    """
    return returncode in (130, 255, 3221225786, -2, 4294967294)
...
def _cleanup_on_interrupt(
    ...
    """Perform cleanup when loop is interrupted.

    Args:
        logger: LoopLogger instance for printing messages
        task_info: TaskInfo to save before exiting
        info_file: Path to info.json file
        iteration_num: Current iteration number
        returncode: Optional return code that triggered interrupt
    """
    logger.print_stderr(f"\n⚠️  Loop interrupted during iteration {iteration_num}")
    if returncode is not None:
        logger.print_stderr(f"   Exit code: {returncode}")
    logger.print_stderr("   Saving task info before exit...")

    # Save task info with interrupt marker
    task_info.end_iteration(returncode or 130, error="Interrupted by user")
    task_info.mark_completed(error="Interrupted by user")
    task_info.save(info_file)

    logger.print_stderr("   Cleanup complete")
...
def _run_loop(args: "Args", claude_path: str, loop_count: int) -> int:
    ...
    # No outer try-except needed - KeyboardInterrupt is handled in runner.py
    with LoopLogger(log_file) as logger:
        for i in range(start_iteration - 1, loop_count):
            ...
            try:
                ...
                # Check if subprocess exited due to interrupt
    ...
    # No outer KeyboardInterrupt handler needed - runner.py handles it
    # Inner handlers (during subprocess execution) do cleanup and re-raise
    return 0
