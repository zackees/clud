...
FORCE_CLEAR: bool = bool(os.environ.get("CLUD_FORCE_CLEAR", "0") == "1")
...
def get_force_remove_image_previous() -> bool:
    ...
    return os.environ.get("CLUD_FORCE_CLEAR", "0") == "1"


def set_clear() -> None:
    os.environ["CLUD_FORCE_CLEAR"] = "1"
...
def get_lock(image_name: str) -> InterProcessLock:
    """Get the inter-process lock for this DockerManager instance."""
    lock_file = CONFIG_DIR / f"{image_name}.lock"
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    print(CONFIG_DIR)
    if not lock_file.parent.exists():
        lock_file.parent.mkdir(parents=True, exist_ok=True)
    out: InterProcessLock
    out = InterProcessLock(str(lock_file))  # type: ignore
    return out
...
def set_force_remove_image_previous(new_value: str | None = None) -> None:
    if new_value is not None:
        os.environ["CLUD_FORCE_CLEAR"] = new_value
    else:
        os.environ["CLUD_FORCE_CLEAR"] = "1"
...
class DockerManager:
    ...
    def ensure_linux_containers_for_windows() -> bool:
        ...
        try:
            ...
            if not _IS_GITHUB:
                answer = input("\nDocker on Windows must be in linux mode, this is a global change, switch? [y/n]").strip().lower()[:1]
                if answer != "y":
                    return False

            print("Switching to Linux containers...")
            warnings.warn("Switching Docker to use Linux container context...", stacklevel=2)
            ...
            warnings.warn(f"Failed to switch to Linux containers with context '{linux_context}': {verify.stdout}", stacklevel=2)
            return False
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            while attempts < max_attempts:
                ...
                print(f"Docker not ready yet, waiting {wait_time:.1f}s... (attempt {attempts}/{max_attempts})")
                time.sleep(wait_time)

            print("Failed to start Docker within the expected time.")
            print("Please try starting Docker Desktop manually and run this command again.")
        except KeyboardInterrupt:
    ...
    def has_newer_version(self, image_name: str, tag: str = "latest") -> tuple[bool, str]:
        """
    ...
    def validate_or_download_image(self, image_name: str, tag: str = "latest", upgrade: bool = False) -> bool:
        """
        ...
        if not ok:
            warnings.warn("Failed to ensure Linux containers on Windows. This build may fail.", stacklevel=2)
            ...
            try:
                ...
                if upgrade:
                    ...
                    try:
                        ...
                        remote_image_hash_from_local_image = None
                        stack = traceback.format_exc()
                        warnings.warn(f"Error getting remote image hash from local image: {stack}", stacklevel=2)
                    if remote_image_hash_from_local_image == remote_image_hash:
                ...
                try:
                    ...
                    warnings.warn(f"Image {image_name}:{tag} not found after download.", stacklevel=2)
        return True
        ...
        try:
            ...
            if not container_image_tags:
                ...
                if container_image_id != current_image.id:
                    print(f"Container using different image version. Container: {container_image_id}, Current: {current_image.id}")
                    return False

            # Check command if specified
            if command and container.attrs["Config"]["Cmd"] != command.split():
                print(f"Command mismatch: {container.attrs['Config']['Cmd']} != {command}")
                return False
            ...
            if volumes_dict:
                ...
                for host_dir, mount in volumes_dict.items():  # type: ignore[misc]
                    ...
                    if container_mounts[host_dir] != mount:
                        print(f"Volume {host_dir} has different mount options: {container_mounts[host_dir]} != {mount}")
                        return False
    ...
    def run_container_detached(
        ...
        if get_force_remove_image_previous():
            # make a banner print
            print("Force removing previous image due to CLUD_FORCE_CLEAR environment variable.")
        ...
        try:
            ...
            if container.status == "running":
                ...
                if container.status == "running":
                    ...
                    print(f"Container {container_name} did not restart within {timeout} seconds.")
    ...
    def build_image(
        ...
        if not build_context.exists():
            raise FileNotFoundError(f"Build context directory not found at {build_context}")
        ...
        try:
            ...
            rtn = proc.wait()
            if rtn != 0:
                warnings.warn(f"Error building Docker image, is docker running? {rtn}, stdout: {stdout}, stderr: {proc.stderr}", stacklevel=2)
...
def main() -> None:
    ...
    try:
        ...
        container = docker_manager.run_container_detached(image_name, tag, container_name, command)
        ...
        if container_or_none is not None:
            ...
            warnings.warn(f"Container {container_name} not found.", stacklevel=2)
