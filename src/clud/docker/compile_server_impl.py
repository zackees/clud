...
from clud.docker.docker_manager import (
    DISK_CACHE,
    Container,
    DockerManager,
    RunningContainer,
...
from clud.emoji_util import EMO
from clud.settings import DEFAULT_CONTAINER_NAME, IMAGE_NAME, SERVER_PORT
from clud.sketch import looks_like_fastled_repo
from clud.types import BuildMode, CompileResult, CompileServerError
from clud.util import port_is_free, print_banner
...
def _try_get_fastled_src(path: Path) -> Path | None:
    ...
    if looks_like_fastled_repo(path):
        print("Looks like a FastLED repo, using it as the source directory and mapping it into the server.")
...
class CompileServerImpl:
    def __init__(
        ...
        if interactive and not mapped_dir:
            raise ValueError("Interactive mode requires a mapped directory point to a sketch")
        if not interactive and mapped_dir:
            warnings.warn(f"Mapped directory {mapped_dir} is ignored in non-interactive mode", stacklevel=2)
        ...
        if allow_libcompile and self.fastled_src_dir is None:
            print(f"{EMO('⚠️', 'WARNING:')}  libfastled compilation disabled: volume source mapping not available")
    ...
    def start(self, wait_for_startup=True) -> None:
        ...
        if self._port != 0:
            warnings.warn("Server has already been started", stacklevel=2)
    ...
    def web_compile(
        ...
        from clud.web_compile import web_compile  # avoid circular import
    ...
    def project_init(self, example: str | None = None, outputdir: Path | None = None) -> None:
        from clud.project_init import project_init  # avoid circular import
    ...
    def running(self) -> tuple[bool, Exception | None]:
        ...
        if not docker_running:
            ...
            if IS_MAC and "FileNotFoundError" in str(err):
                traceback.print_exc()
    ...
    def port(self) -> int:
        if self._port == 0:
            warnings.warn("Server has not been started yet", stacklevel=2)
        return self._port
    ...
    def ping(self) -> bool:
        try:
            response = httpx.get(f"http://localhost:{self._port}", follow_redirects=True)
    ...
    def _start(self) -> int:
        ...
        if not running and not self.docker.start():
        ...
        updated = self.docker.validate_or_download_image(image_name=IMAGE_NAME, tag="latest", upgrade=upgrade)
        ...
        if not self.interactive:
            ...
            if client_port_mapped and free_port:
                warnings.warn(f"Can't expose port {INTERNAL_DOCKER_PORT}, disabling port forwarding in interactive mode", stacklevel=2)
                ports = {}
    ...
    def get_emsdk_headers(self, filepath: Path) -> None:
        ...
        from clud.util import download_emsdk_headers
