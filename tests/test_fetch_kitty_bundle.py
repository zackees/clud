...
PINNED_URL = (
    ...
    "kitty-windows-compat-0be809bc/WezTerm-windows-portable.zip"
)
PINNED_SHA256 = "933c3a0ed459b9b19dcb79b47e94d8583866271fa8fb7a0c2121aab1bcc82b5a"
...
def test_native_windows_integration_smokes_installed_gui() -> None:
    ...
    assert '"bundle/target/$($manifest.profile_dir)/mock-agent.exe"' in workflow
    assert "-MockAgentPath $mockAgent" in workflow
    ...
    assert workflow.index("uses: ./.github/actions/setup-exec") < workflow.index(
        "ci/kitty_windows_smoke.ps1"
    )
    assert workflow.index("--reinstall --no-deps $wheel.FullName") < workflow.index(
        "ci/kitty_windows_smoke.ps1"
    )
    assert workflow.index("ci/kitty_windows_smoke.ps1") < workflow.index("-m ci.run_bundle")
