...
Both scripts install [`uv`](https://docs.astral.sh/uv) if needed, then `uv tool install clud`, and put `clud` on PATH for new shells. Pin a version with `CLUD_VERSION=2.0.12 curl ... | sh` (POSIX) or `$env:CLUD_VERSION = '2.0.12'; irm ... | iex` (PowerShell). Re-run to upgrade.
