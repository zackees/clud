...
Your shell is capped by clud's hook to `gh pr`, `gh run view|rerun`, read-only git, `git push` and `pr_merge_watch`; in feature-branch mode also `gh label create grind:on-feature`, `gh issue edit <n> --add-label grind:on-feature`, `gh issue comment`, and `gh pr edit <feature-pr> --body`.
After a goal PR merges into a feature branch: label the goal issue and the meta `grind:on-feature`, comment with the `<!-- grind:v1 feature-pr=#<fpr> branch=<b> goal-pr=#<gpr> run=<id> -->` marker, and add `Closes #<N>` plus the goals-table row to the feature PR body. Never `gh issue close`; the hook denies it.
