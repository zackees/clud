...
1. **Meta issue URL or number.** A parent issue with native sub-issues
   ...
   issue references. Each open child becomes a goal. `/grind` never closes
   the parent itself, and no child's PR closes it; GitHub's sub-issue
   tracking (or the user) closes it.
...
4. **Nothing.** List the repo's open issues (`gh issue list`), excluding
   ones with an open linked PR and ones labelled `grind:on-feature` or
   `grind:followup`
   (`gh issue list --search '-label:grind:on-feature -label:grind:followup'`),
