...
- `tool_shell_lifecycle_windows.rs` — Windows-only #616 live Job completion-port coverage: a registered fake agent's direct leaked client is reaped while an intentionally detached nested `cmd.exe` survives until explicit cleanup.
