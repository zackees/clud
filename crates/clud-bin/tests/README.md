...
- `pty/toast_pty.rs` — #1189 toasts in real PTY sessions through the composited writer (`run_raw_pty_pump_with_toasts_for_test`): the kitty tier uploads, places, removes and frees the toast image with `q=2` while child text replays intact, and the title fallback pushes/pops the title stack (Linux, macOS, Windows); the alternate-screen text tier repaints exactly what the child drew (Unix).
...
- `runtime_cache_hop_windows.rs` - Windows-only #333 regression: the runtime-cache relay must leave a real detached daemon alive after the relay exits, proving it adds no `KILL_ON_JOB_CLOSE` containment.
