...
pub const ANTHROPIC_COMPAT_PROVIDERS: &[AnthropicCompatProvider] = &[
    AnthropicCompatProvider {
        ...
        // DeepSeek's documented Claude Code profile puts the auto-context
        // Flash model in the haiku/subagent slots. This is the compiled-in
        // fallback; `server_settings` serves the live name (#1192).
        subagent_wire_id: "deepseek-flash",
        role_models: None,
    ...
    AnthropicCompatProvider {
        ...
        // Unlike DeepSeek (which puts the auto-context `deepseek-flash` in the
        // haiku/subagent slots), Kimi's official Claude Code profile points
        // the haiku AND subagent slots at the same main model: kimi-k3[1m].
...
mod tests {
    ...
    fn kimi_subagent_wire_id_points_at_the_same_model_unlike_deepseek() {
        ...
        // DeepSeek's: haiku/subagent slots carry the SAME 1m wire ID as the
        // main model, while DeepSeek's documented profile puts the
        // auto-context `deepseek-flash` there instead of `deepseek-flash[1m]`.
        let deepseek = descriptor_for(ModelProvider::DeepSeek).unwrap();
        let kimi = descriptor_for(ModelProvider::Kimi).unwrap();
        assert_eq!(kimi.subagent_wire_id, "kimi-k3[1m]");
        assert_eq!(deepseek.subagent_wire_id, "deepseek-flash");
        assert_ne!(
            deepseek.subagent_wire_id,
            crate::provider_catalog::reviewed_default_model(ModelProvider::DeepSeek)
                .unwrap()
                .wire_id
        );
