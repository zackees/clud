    ...
    for request in server.incoming_requests() {
        ...
        if path.starts_with("/v1/") {
            ...
            match (method.clone(), path.as_str()) {
                ...
                (Method::Get, "/v1/openapi.json") => respond_json(request, 200, &serde_json::to_vec(&openapi_document()).unwrap_or_else(|_| b"{}".to_vec())),
...
fn openapi_document() -> serde_json::Value {
    let mut document: serde_json::Value = serde_json::from_str(OPENAPI_JSON).expect("embedded OpenAPI is valid JSON");
    let id_parameter = serde_json::json!({"name":"id","in":"path","required":true,"schema":{"type":"string"}});
    for path in ["/v1/sessions/{id}/interrupt", "/v1/sessions/{id}/turns", "/v1/sessions/{id}/events"] {
        document["paths"][path]["parameters"] = serde_json::Value::Array(vec![id_parameter.clone()]);
    }
