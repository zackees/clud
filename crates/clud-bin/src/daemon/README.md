...
- `conhost_reaper.rs` — issue #539: bounded reaper for orphaned Windows `conhost.exe` processes left behind by codex tool-subprocess spawns. `select_orphaned_conhosts` is a pure decision function over a `{pid, ppid, name, start_time}` snapshot (dead-parent + PID-reuse-guard orphan definition, exhaustively unit-tested with mock data); `sweep_once` pairs it with real `sysinfo` enumeration/termination and is a no-op off Windows. Wired into the daemon via `server.rs::spawn_conhost_reap_sweeper` on a dedicated 60s-cadence thread; each reap is logged as `conhost_reap_finished` in `daemon-events.jsonl`.
