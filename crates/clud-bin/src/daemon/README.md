...
- `api_session_lifecycle.rs` — #1043 serialized logical-session lifecycle: one active API generation, durable idempotency replay/conflict, graceful interrupt with bounded forced fallback, replace sequencing, and terminal kill. It does not alter interactive attach Ctrl-C or legacy daemon interrupt RPC behavior.
