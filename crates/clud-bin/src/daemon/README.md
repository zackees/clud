...
- `server.rs` — daemon-process entry: binds the loopback listener, applies `DaemonRuntimeConfig`, spawns the GC registry worker, accepts `Create`/`Session`/`Terminate`/`Gc` requests, spawns worker subprocesses, reaps them.
