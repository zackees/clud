...
- `prompts.rs` — static prompt templates (`FIX_PROMPT`, `GITHUB_FIX_TEMPLATE`, `DO_GOAL_TEMPLATE`, `REBASE_PROMPT`, `UP_PROMPT`) and the backend-aware `push_prompt`, `build_up_prompt`, `build_fix_prompt`, `build_do_prompt` builders. `do` keeps `/goal` as its primary contract and selects the bundled `/clud-meta-work` playbook only for genuinely independent deliverables. For a detected meta issue, URL-based `/do` / `$do` requires `AskUserQuestion` and waits for its answer before parallelizing; explicit `/meta-issue` / `$meta-issue` assumes parallel execution is already approved. Both URL and free-form contracts require a clean checkout rebased to local origin.
