...
- `prompts.rs` — static prompt templates (`FIX_PROMPT`, `GITHUB_FIX_TEMPLATE`, `DO_GOAL_TEMPLATE`, `REBASE_PROMPT`, `UP_PROMPT`) and the backend-aware `push_prompt`, `build_up_prompt`, `build_fix_prompt`, `build_do_prompt` builders. `do` keeps `/goal` as its primary contract and routes genuinely independent deliverables to `/grind` ([grind.md](../../../../docs/architecture/grind.md)). `build_grind_prompt` seeds `/grind <url>`. For a detected meta issue, `clud do` asks before parallelizing while explicit `/meta-issue` assumes approval; every child retains its own branch and one or more separately validated, watched, and merged PRs. The URL contract closes every referenced issue and rebases the clean starting checkout to origin's main branch; the free-form contract likewise returns clean to its starting branch.
