...
- `prompts.rs` — static prompt templates (`FIX_PROMPT`, `GITHUB_FIX_TEMPLATE`, `REBASE_PROMPT`, `UP_PROMPT`) and the backend-aware `push_prompt`, `build_up_prompt`, `build_fix_prompt`, `build_do_prompt` builders. `build_do_prompt` seeds `/goal /do <target>`, or `/goal /grind <url>` for a meta issue; the contract lives in the bundled `/do` skill. `build_grind_prompt` seeds `/grind <url>`.
- `do_prompt.rs` — `clud do-prompt <target>`, the renderer behind the `/do` skill (#1322): the starting-branch verdict (`default`, `in_progress`, `stale_merged`, `stale_empty`, `dirty`, `unrelated`, `blocked`, `unknown`) from `git`/`gh`, plus the issue or free-form contract, a `/grind` redirect for meta issues, or a stop. Tested end to end in `tests/harness/test_do.py`.
...
- `build_do_prompt(target, meta) -> String` — `prompts.rs`
