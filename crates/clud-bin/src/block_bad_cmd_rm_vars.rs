use super::*;
use crate::deletion_policy::{normalized_component_count, unsafe_delete_base_reason};
...
/// Reuse the source interpreter's quote-aware/background-aware splitter.
pub(super) fn identity_statements(command: &str) -> Result<Vec<&str>, ()> {
    let statements = split_statements(command)?;
    if contains_unsupported_control_flow(&statements) {
        return Err(());
    }
    Ok(statements.into_iter().map(|s| s.text).collect())
}
