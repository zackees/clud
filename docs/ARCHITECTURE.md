...
| [architecture/server-settings.md](architecture/server-settings.md) | ~130 | Server-side settings (#1192): the baked-in `server-settings.json` refreshed from `main`, per-section last-known-good merge, bounded-wait refresh and backoff, strict JSON parsing, controls, the scheduled `model_contexts` window map from OpenRouter's datasheet (#1258), and how to change or add a setting |
