...
`clud` is one binary; the daemon and the worker are the same exe re-entered with hidden internal subcommands. The daemon is detached by `spawn_detached_daemon` in `client.rs`, through running-process's daemon spawn (#1186); client invocation is in `ensure_daemon`.
...
2. `run_centralized_session` calls `ensure_daemon` which probes the port in `daemon.json`; if no live daemon answers, it spawns `current_exe` with argv `["__daemon", "--state-dir", <path>]` via `spawn_detached_daemon` (running-process `spawn_daemon_breaking_away_with_env_policy` with `EnvironmentPolicy::Inherit` and the originator tag removed, #1186) and waits up to 5s for the new daemon to write `daemon.json` and accept a probe. A newer live daemon is never replaced by an older client: the client prints a yellow compatibility error and exits 1.
...
7. On the client, a reader thread parses `Output` / `Exited` / `Error` and writes to stdout. An interactive PTY attach first pushes the same kitty keyboard frame as the local pump (`session::KeyboardEnhancementGuard`), and the reader feeds each `Output` chunk to its child-frame tracker before writing it (`relay_worker_output`). The relay then strips the child's OSC 0/2 title writes with one `console_title::OscTitleStripper` per attach, as the local pump does, so a title cannot overwrite clud's stamped console title; the worker keeps the raw bytes for its backlog, log and transcript (#1372). `attach_to_session` holds that guard until the reader thread has joined, past raw mode's end, so on every exit (session end, detach, Ctrl+C, panic) it pops the frames the child left pushed and then its own (#1363, #1221). In parallel, `run_remote_interactive` (`attach.rs`) forwards the terminal's input as raw bytes, mirroring the local PTY pump. It starts the byte source (`attach_input::RawInput`: the `console_input` reader on Windows, polled stdin elsewhere), then VT input, then raw mode (`RawTerminalGuard`, `types.rs`), and runs each chunk through `RemoteInputFilter` (`attach_input.rs`):
   ...
   - Ctrl+V with an image on the clipboard pastes the image's saved path, as in the local pump (#1373). One layer expands it per source, never two: the Windows `console_input` reader already does (`adapt_event`), and for a byte-stream source (POSIX stdin, or the Windows fallback reader) the filter runs `paste_image::expand_ctrl_v_bytes` after the Ctrl+C check (`RawInput::expands_ctrl_v`). Only the legacy `0x16` byte pastes; a kitty CSI u Ctrl+V (press or release) is forwarded unchanged, as in the local pump.
   - A terminal-size change seen on the 25 ms tick → `WorkerClientMessage::Resize`.
   - The loop also ends when the session does (#1448). The reader thread records the worker's `Exited` code (or 1 after a worker `Error`) only once it has relayed the final `Output`, and the loop reads that record on every tick, right after the interrupt flag: it releases a held Esc, then returns `LocalAttachResult::Completed(code)`, so `attach_to_session` joins the reader and returns the session's exit code instead of waiting for Ctrl+C or stdin EOF. The non-interactive attach reads the same record in `wait_for_remote_or_interrupt`. The loop's decisions live in `pump_remote_input`, behind the `RemoteAttachIo` seam (terminal input, worker socket, voice/resize/drag-and-drop tick) that its unit tests script; `tests/integration/test_daemon_interactive_attach.py` covers it end to end on a pseudo-terminal.
